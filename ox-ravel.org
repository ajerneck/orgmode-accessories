      ...
      (let
          ((outfile  (org-export-output-file-name ".Rnw" subtreep)))
           (org-export-to-file 'latex-noweb 
                               ...
                               body-only ext-plist)
           (message (format "compiling %s to pdf using knit2pdf..." outfile))
           (message "Remember 1) #+PROPERTY: :session *R* 2) ess-switch-process 3) library(knitr)if you get errors.")
           (ess-execute (format "knit2pdf('%s')" outfile))  
           )))
#+END_SRC
